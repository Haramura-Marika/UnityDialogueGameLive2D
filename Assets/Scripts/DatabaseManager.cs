...
public class DatabaseManager : MonoBehaviour
{
        ...
        try
        {
            using (var conn = new SqliteConnection(connectionString))
            {
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "ALTER TABLE chat_history ADD COLUMN affinity INTEGER DEFAULT 50;";
                        cmd.ExecuteNonQuery();
                    }
                }
                catch
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "ALTER TABLE chat_history ADD COLUMN mood INTEGER DEFAULT 50;";
                        cmd.ExecuteNonQuery();
                    }
                }
                catch
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "ALTER TABLE chat_history ADD COLUMN energy INTEGER DEFAULT 50;";
                        cmd.ExecuteNonQuery();
                    }
                }
                catch
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "ALTER TABLE chat_history ADD COLUMN stress INTEGER DEFAULT 50;";
                        cmd.ExecuteNonQuery();
                    }
                }
                catch
                ...
                try
                {
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "ALTER TABLE chat_history ADD COLUMN trust INTEGER DEFAULT 50;";
                        cmd.ExecuteNonQuery();
                    }
                }
                catch
    ...
    public void SaveChatHistory(string username, string characterName, List<ChatMessage> chatHistory, int affinity = 50,
        int mood = 50, int energy = 50, int stress = 50, int trust = 50, string slotName = "autosave")
    {
        try
        {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            int affinity = reader.IsDBNull(3) ? 50 : reader.GetInt32(3);
                            int mood = reader.IsDBNull(4) ? 50 : reader.GetInt32(4);
                            int energy = reader.IsDBNull(5) ? 50 : reader.GetInt32(5);
                            int stress = reader.IsDBNull(6) ? 50 : reader.GetInt32(6);
                            int trust = reader.IsDBNull(7) ? 50 : reader.GetInt32(7);

                            var history = JsonConvert.DeserializeObject<List<ChatMessage>>(chatDataJson);

                            Debug.Log(
                                $"[DatabaseManager] 已加载用户 {username} 的对话历史 (角色: {characterName}, 时间: {timestamp}, 好感: {affinity}, 心情: {mood}, 精力: {energy}, 压力: {stress}, 信任: {trust})");
