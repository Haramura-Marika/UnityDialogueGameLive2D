...
namespace AI.Providers
{
    ...
    internal static class ChatConfig
    {
        ...
        public static string GetGroupId(string serviceProp)
        {
            var cfg = GetServiceConfig(serviceProp);
            if (cfg != null)
            {
                var groupIdProp = cfg.GetType().GetProperty("GroupId", BindingFlags.Public | BindingFlags.Instance);
                var val = groupIdProp?.GetValue(cfg, null) as string;
                if (!string.IsNullOrEmpty(val)) return val;
            }
            return null;
        }
